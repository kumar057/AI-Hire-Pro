...
export function CompanyDashboardHome() {
  ...
  useEffect(() => {
    void companyService.getDashboard().then(setData);
  }, []);
    ...
    <div className="mt-6 space-y-6">
        ...
        <div className="grid gap-6 lg:grid-cols-[1fr_22rem] lg:items-center">
          <div>
            ...
            <p className="mt-4 max-w-3xl leading-7 text-slate-600 dark:text-slate-300">
              Monitor hiring performance, candidate flow, active jobs, and recruiter priorities from
          ...
          <div className="rounded-lg border border-emerald-200 bg-emerald-50 p-5 dark:border-emerald-300/20 dark:bg-emerald-300/10">
            <div className="flex items-center gap-3">
              <FiAward className="size-6 text-emerald-600" />
              <span className="font-bold text-emerald-800 dark:text-emerald-200">
                {data.summary.premium_status}
              </span>
            </div>
            <p className="mt-3 text-sm text-emerald-700 dark:text-emerald-200">
              Premium hiring tools active
            </p>
            <div className="mt-4 flex items-center gap-2 font-bold text-emerald-700">
              <FiArrowUpRight />
              {data.summary.monthly_growth}% monthly growth
            </div>
          </div>
        </div>
      </motion.section>
      <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        {COMPANY_METRICS.map((metric, index) => {
          ...
          const value = data.summary[metric.key as keyof CompanyDashboardSummary];
          return (
            <motion.article
              animate={{ opacity: 1, y: 0 }}
            ...
            >
              ...
              <p className="mt-1 font-bold text-slate-700 dark:text-slate-200">{metric.label}</p>
        ...
        <section className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm dark:border-white/10 dark:bg-slate-900/70">
          <div className="flex items-center justify-between">
            <div>
              ...
              <h2 className="mt-2 text-xl font-bold">Six-month growth</h2>
            </div>
          ...
          <div className="mt-5 h-72">
            <ResponsiveContainer height="100%" width="100%">
              <AreaChart data={trend}>
                <defs>
                  <linearGradient id="companyTrend" x1="0" x2="0" y1="0" y2="1">
        ...
        <section className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm dark:border-white/10 dark:bg-slate-900/70">
          ...
          <div className="mt-5 space-y-5">
            {data.activity.map((item) => (
              <div className="border-l-2 border-cyan-500 pl-4" key={item.title}>
      ...
      <DashboardFeedWidgets root="/company/dashboard" />
    </div>
  );
}
