...
export function SavedJobsPage() {
  ...
  function handleRemoveSaved(job: JobPosting) {
  ...
  return (
    <div className="mt-6 space-y-6">
      <header className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm dark:border-white/10 dark:bg-slate-900/70">
        ...
        <h1 className="mt-2 text-3xl font-bold text-slate-950 dark:text-white">Saved role queue</h1>
        <p className="mt-3 max-w-3xl text-sm leading-6 text-slate-600 dark:text-slate-300">
          Review saved dummy roles, filter the queue, remove items, and submit placeholder
          applications.
        </p>
      </header>
      <JobToolbar
        ...
        workMode={workMode}
      />
      <JobTable jobs={visibleJobs} mode="saved" onApply={handleApply} onRemoveSaved={handleRemoveSaved} />
